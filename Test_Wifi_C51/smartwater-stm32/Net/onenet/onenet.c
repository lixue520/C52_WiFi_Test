...
extern u8 IC_recharge_money;//充值标志,有上位机或者串口屏幕控制
extern double recharge;//充值金额
extern unsigned char user_name[16]; //IC卡持有者
    ...
    if(MQTT_PacketConnect(ProductID, Auth_Info, DeviceID, 256, 0, MQTT_QOS_LEVEL0, NULL, NULL, 0, &mqttPacket) == 0)
    {
        ...
        if(dataPtr != NULL)
        {
            if(MQTT_UnPacketRecv(dataPtr) == MQTT_PKT_CONNACK)
            {
                ...
                case 1:
                    DEBUG_LOG("WARN:	连接失败：协议错误\r\n");
                    break;
                case 2:
                    DEBUG_LOG("WARN:	连接失败：非法的clientid\r\n");
                    break;
                case 3:
                    DEBUG_LOG("WARN:	连接失败：服务器失败\r\n");
                    break;
                case 4:
                    DEBUG_LOG("WARN:	连接失败：用户名或密码错误\r\n");
                    break;
                case 5:
                    DEBUG_LOG("WARN:	连接失败：非法链接(比如token非法)\r\n");
                    break;

                default:
                    DEBUG_LOG("ERR:	连接失败：未知错误\r\n");
                    break;
                }
            }
        }
...
{
    MQTT_PACKET_STRUCTURE mqttPacket = {NULL, 0, 0, 0};    //协议包
    const char *new_msg = msg + 2;                          //忽略掉前面两个字节
            ...
            else
            {
                ...
                if(strstr(json_value->valuestring,"ZW_Led") != NULL)
                {
                    ...
                    if(json_value->valueint) {
                        ...
                        GPIO_SetBits(GPIOB,GPIO_Pin_7); //紫外线开
                ...
                if(strstr(json_value->valuestring,"Beep") != NULL)
                {
                    ...
                    if(json_value->valueint) {
                        ...
                        GPIO_ResetBits(GPIOB, GPIO_Pin_5);//报警，关
                ...
                if(strstr(json_value->valuestring,"DZF") != NULL)
                {
                    ...
                    if(json_value->valueint) {
                       ...
                       GPIO_ResetBits(GPIOB,GPIO_Pin_8);//电磁阀1 
                ...
                if(strstr(json_value->valuestring,"DZF1") != NULL)
                {
                    json_value = cJSON_GetObjectItem(json, "value");
                    if(json_value->valueint) {
                        GPIO_SetBits(GPIOC,GPIO_Pin_13);//电磁阀2
                        DEBUG_LOG("Tips:	DZF1 is oprn OK\r\n");

                    }
                    else {

                        GPIO_ResetBits(GPIOC,GPIO_Pin_13);//电磁阀2
                ...
                if(strstr(json_value->valuestring,"Pump") != NULL)
                {
                    ...
                    if(json_value->valueint) {
                        ...
                        GPIO_ResetBits(GPIOA,GPIO_Pin_11);//水泵1，关
                        GPIO_ResetBits(GPIOA,GPIO_Pin_12);
                ...
                if(strstr(json_value->valuestring,"Pump1") != NULL)
                {
                    ...
                    if(json_value->valueint) {
                        ...
                        GPIO_ResetBits(GPIOB,GPIO_Pin_3);//水泵2，关
                        GPIO_ResetBits(GPIOB,GPIO_Pin_4);
